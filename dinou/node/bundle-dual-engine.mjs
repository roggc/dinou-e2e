// dinou/node/bundle-dual-engine.mjs
// Reusable Dual-Bundle Compiler for Dinou:
// Compiles Pass A (RSC Engine) and Pass B (SSR Engine) using esbuild
// in a single process without React collisions.
...
export async function bundleDualEngine(options = {}) {
  ...
  const routeModulesPath = path.join(nodeDir, "route-modules.mjs");
...
`;
  const envSetupPath = path.join(nodeDir, "env-setup.mjs");
  fs.writeFileSync(envSetupPath, envSetupContent, "utf8");

  // 5. Generate ssr-client-manifest.mjs
  let ssrManifestCode = `// Auto-generated client modules registry for Native Node SSR\n`;
  ...
  clientComponents.forEach((compPath, index) => {
    ...
    const altFileUrl = fileUrl.replace(
      /file:\/\/\/([a-zA-Z]):/,
      (m, d) => "file:///" + (d === d.toLowerCase() ? d.toUpperCase() : d.toLowerCase()) + ":"
    );
  ...
  serverFunctionFiles.forEach((relPath, index) => {
    ...
    const altFullFileUrl = fullFileUrl.replace(
      /file:\/\/\/([a-zA-Z]):/,
      (m, d) => "file:///" + (d === d.toLowerCase() ? d.toUpperCase() : d.toLowerCase()) + ":"
    );
  ...
  ssrManifestCode += `  },\n  moduleLoading: null,\n};\n`;
  const ssrClientManifestPath = path.join(nodeDir, "ssr-client-manifest.mjs");
  fs.writeFileSync(ssrClientManifestPath, ssrManifestCode, "utf8");

  // 6. Entry Files Generation
  const rscEntryContent = `// Auto-generated RSC Engine entry for Node.js (Dual-Bundle)
...
`;
  ...
  const ssrEntryContent = `// Auto-generated SSR Engine entry for Node.js (Dual-Bundle)
...
`;
  const ssrEntryPath = path.join(nodeDir, "ssr-entry.mjs");
  fs.writeFileSync(ssrEntryPath, ssrEntryContent, "utf8");

  // 7. Esbuild Configuration & Compilation
  const nodeBuiltins = [
...
`,
  ...
  const rscOutfile = path.join(nodeDir, "rsc-engine.mjs");
  const ssrOutfile = path.join(nodeDir, "ssr-engine.mjs");
  ...
  return {
    nodeDir,
    rscEnginePath: rscOutfile,
    ssrEnginePath: ssrOutfile,
    rscEntryPath,
    ssrEntryPath,
...
export default { bundleDualEngine };
